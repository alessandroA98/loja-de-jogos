...
import com.lojajogos.repository.CategoriasRepository;
...
public class CategoriasController {
	...
	@GetMapping("/id")
	public ResponseEntity<Categorias> geById(@PathVariable Long id){
		return categoriasRepository.findById(id).map(resposta -> ResponseEntity.ok(resposta))
				.orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build());
	}
	@GetMapping("/nome/{nome}")
	public ResponseEntity<List<Categorias>> getByCategorias(@PathVariable String nome){
		return ResponseEntity.ok(categoriasRepository.findAllByDescricaoContainingIgnoreCase(nome));
	}
	@PostMapping
	...
	@PutMapping
	public ResponseEntity<Categorias> pu(@Valid @RequestBody Categorias categorias){
