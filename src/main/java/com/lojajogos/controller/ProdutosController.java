...
import com.lojajogos.model.Produtos;
import com.lojajogos.repository.ProdutosRepository;
...
public class ProdutosController {
	...
	@GetMapping("/id")
	public ResponseEntity<Produtos> geById(@PathVariable Long id){
		return produtosRepository.findById(id).map(resposta -> ResponseEntity.ok(resposta))
				.orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build());
	}
	@GetMapping("/nome/{nome}")
	public ResponseEntity<List<Produtos>> getByProdutos(@PathVariable String nome){
